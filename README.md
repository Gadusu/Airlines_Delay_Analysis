...
A logistic regression model was trained to predict the binary outcome of whether a flight would be delayed or on-time, based on features such as arrival delay, carrier delay, and weather delay.
...
#### 2\. Decision Tree: 
A classification tree model was also developed using the rpart (Recursive Partitioning and Regression Trees) algorithm. This model aims to predict flight delays by recursively partitioning the data based on the input features.
...
##### 1\. Year-wise Delays: 
The visualization of departure delays by year revealed variations in the number of delays across different years. This information could be valuable for airlines and aviation authorities to identify potential trends and patterns in flight delays over time.
...
##### 2\. Carrier-wise Delays: 
The analysis also highlighted differences in departure delays across different airline carriers. Some carriers experienced higher rates of delays compared to others, indicating potential areas for improvement in operational efficiency.
...
##### 3\. Reason for Delays: 
The primary reasons for flight delays were carrier delays, weather delays, and delays caused by the National Airspace System (NAS). The "Reason" column, created during data preprocessing, showed that the majority of delays were attributed to these three factors.
